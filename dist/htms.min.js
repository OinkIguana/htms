...
function run(block, scope) {
  return Array.prototype.reduce.call(block, ($_, elem) => {
    ...
    switch (elem.nodeName) {
      ...
      case 'B':
        // boolean
        return new Boolean(run(elem.childNodes, _extends({}, scope)));
      case 'DEL':
      ...
      case 'SUP':
        // exponentiation
        return new Number(Math.pow($_, run(elem.childNodes, _extends({}, scope))));
      case 'SMALL':
        // less than
        return new Boolean(v($_) < v(run(elem.childNodes, _extends({}, scope))));
      case 'SAMP':
        // equality
        return new Boolean(v($_) === v(run(elem.childNodes, _extends({}, scope))));
      case 'ARTICLE':
        ...
        if (b.valueOf()) {
          return ct ? run(ct.childNodes, _extends({}, scope)) : $_;
        } else if (cf) {
          return cf ? run(cf.childNodes, _extends({}, scope)) : $_;
        }
      case 'INS':
        // function call
